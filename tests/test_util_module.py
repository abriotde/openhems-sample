...
class TestUtilModule(unittest.TestCase):
    ...
    def test_configurations_manager(self):
        """
        ...
        if saved_file.exists():
            saved_file.unlink()
        configurator.save(saved_file)
        reference_file = cur_folder / 'data/openhems_test_save.yaml'
        with open(reference_file, encoding="utf-8") as f1, \
                open(saved_file, encoding="utf-8") as f2:
        ...
        new_dict = ConfigurationManager.toTree(conf)
        self.assertEqual("{'al': [1, 2, 3], 'key0': {'key1': 123, 'toto': 'alpha'}}",str(new_dict))

    def test_cast_utility(self):
        """
        Test Cast
        """
