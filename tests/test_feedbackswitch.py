...
class TestFeedbackSwitch(utils.TestStrategy):
    ...
    def test_feedbackswitch_cycle(self):
        ...
        for i in range(13):
            print(i," : ",pump)
            self.check_values(nodes_ids, [280], isOn=[True])
            self.loop(self.random_dt())
            pump._sensor.value += 1
        print("Reach max : ", pump._sensor.value)
        for i in range(11):
            print(i," : ",pump)
            self.check_values(nodes_ids, [0], isOn=[False])
            self.loop(self.random_dt())
            pump._sensor.value -= 1
