...
Then you will need to configure it. See dedicated page :doc:`configure`. After configured, restart at least OpenHEMS add-on.

Install from docker
-------------------

This is a modern, quick and secure way to install it but it is not for people who want to developp it.
...
In $OPENHEMS_CONFIGPATH you will put openhems.yaml. This is the configuration file of OpenHEMS. 
This configuration is not critical to start OpenHEMS you can start the application and use its helper. But it's important to connect it to Home-Assistant to OpenHEMS. To do this, fill "api" section on openhems.yaml, refer you to chapter installation on :doc:`installation_from_scratch`.
You can find documentation for this on :doc:`configure` but there is an example on [repository](https://raw.githubusercontent.com/abriotde/openhems-sample/refs/heads/main/config/openhems.yaml) to help you. 
...
For a complete installation with an installation of docker and a persistant start of docker, see :doc:`installation_from_scratch` page and configure it to use docker.
